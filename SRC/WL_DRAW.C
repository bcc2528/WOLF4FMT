...
void VGAClearScreen (void)
{
	...
	unsigned int offset = vbuf >> 2;
	vram += offset;
	unsigned int offset_add = (SCREENWIDTH - viewwidth) >> 2;
	unsigned int height = viewheight >> 1;
	vram2 += offset + (height * (SCREENWIDTH >> 2));
	offset = viewwidth >> 2;
	unsigned int x;

...
int             godmode,singlestep,extravbls=0;            // to remove flicker (gray stuff at the bottom)
...
int                     buttonscan[NUMBUTTONS] =
						{sc_Control,sc_Alt,sc_RShift,sc_Space,sc_1,sc_2,sc_3,sc_4,sc_None,sc_None,sc_Escape};
int                     buttonmouse[4]={bt_attack,bt_strafe,bt_nobutton,bt_nobutton};
int                     buttonjoy[4]={bt_attack,bt_strafe,bt_nobutton,bt_use};
...
//      short mousexmove,mouseymove;
		...
		IN_MouseMove(mousexmove, mouseymove);
...
void PollJoystickMove (void)
{
		...
		else if (buttonstate[bt_run] || joypadenabled)
		{
				if (joyx > 64)
						controlx += RUNMOVE*tics;
				else if (joyx < -64)
...
void PlayLoop (void)
{
		...
		if (MousePresent)
		{
			IN_MouseClear();  // Clear accumulated mouse movement
		}

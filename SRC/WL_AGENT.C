...
void StatusDrawPic (unsigned x, unsigned y, unsigned picnum)
{
		...
		vbuf = PAGE1START+(200-STATUSLINES)*SCREENWIDTH;
		LatchDrawPic (x,y,picnum);
		vbuf = PAGE2START+(200-STATUSLINES)*SCREENWIDTH;
		LatchDrawPic (x,y,picnum);
		vbuf = PAGE3START+(200-STATUSLINES)*SCREENWIDTH;
		LatchDrawPic (x,y,picnum);
...
//
		...
		dist = _max(dx,dy); //dist = dx>dy ? dx:dy;
		if (dist<2)
				damage = US_RndT() / 4;
		else if (dist<4)
				damage = US_RndT() / 6;

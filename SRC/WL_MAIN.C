...
void ReadConfig(void)
{
		...
		//
				...
				read(file, &lowres, sizeof(lowres));
				...
				if ((sds == sds_MonoPCM && !PCMPresent) ||
						(sds == sds_StereoPCM && !StereoPCMPresent))
						sds = sds_Off;
...
void WriteConfig(void)
{
		...
		if (file != -1)
		{
				...
				write(file, &lowres, sizeof(lowres));
...
boolean LoadingBar(unsigned current, unsigned total)
{
...
//
		if (tedlevel)
		{
				...
				for (i = 1;i < myargc;i++)
				{
						if ( (level = US_CheckParm(myargv[i],ParmStrings_wl_main)) != -1)
						{

...
void RecursiveConnect (int areanumber)
{
		int     i;
		byte *area;
		area = &areaconnect[areanumber][0];

		for (i=0;i<NUMAREAS;i++)
		{
				if (area[i] && !areabyplayer[i])
				{
						areabyplayer[i] = true;
						RecursiveConnect (i);
				}
...
//
		area = *(mapsegs[0] + (doorptr->tiley<<mapshift)
						+doorptr->tilex)-AREATILE;
		if (areabyplayer[area])
		{
				PlaySoundLocTile(CLOSEDOORSND,doorptr->tilex,doorptr->tiley); // JAB
		}

		doorptr->action = dr_closing;
//
...
void DoorOpening (int door)
{
		...
		//
				map = mapsegs[0] + (doorptr->tiley<<mapshift)
						+doorptr->tilex;
				...
				if (area1 < NUMAREAS && area2 < NUMAREAS)
				{
						...
						if (areabyplayer[area1])
								PlaySoundLocTile(OPENDOORSND,doorptr->tilex,doorptr->tiley);  // JAB
				}
		}
...
//
		...
		//
				position = 0xffff;
				doorptr->ticcount = 0;
				doorptr->action = dr_open;
				actorat[doorptr->tilex][doorptr->tiley] = 0;
		}
...
//
		...
		//
				...
				doorptr->action = dr_closed;

				map = mapsegs[0] + (doorptr->tiley<<mapshift) + doorptr->tilex;

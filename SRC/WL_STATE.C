...
boolean CheckLine (objtype *ob)
{
		...
		if (abs(xt2-xt1) > 0)
		{
				...
				ystep = _min(0x7fffl , _max(((long)(y2-y1)<<8)/abs(x2-x1), -0x7fffl));
				...
				do
				{
						value = (unsigned)tilemap[xy][yfrac>>8];
						yfrac += ystep;
						...
						if (yfrac-ystep/2>doorposition[value])
								return false;
		...
		if (abs(yt2-yt1) > 0)
		{
				...
				xstep = _min(0x7fffl , _max(((long)(x2-x1)<<8)/abs(y2-y1), -0x7fffl));
				...
				do
				{
						value = (unsigned)tilemap[xfrac>>8][xy];
						xfrac += xstep;
						...
						if (xfrac-xstep/2>doorposition[value])
								return false;
				} while (xy != yt2);
		}
